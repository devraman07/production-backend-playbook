...
export const serializeProduct = (req, res) => {
         try {
            const product = products[0];

            const serialize = JSON.stringify(product);
...
export const desearilizeProduct = (req, res) => {
  try {
    ...
    if(!raw) {
      return res.status(400).json({
        success: false,
        data: null,
        error: "Raw data is required for deserialization",
      });
    }
    ...
    return res.status(400).json({
      success: false,
      data: null,
      error: "Invalid JSON format for deserialization",
    });
  }
}